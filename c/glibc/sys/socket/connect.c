 ...
 *
 *	int connect (int socket, struct sockaddr *addr, socklen t length) [Function]
 *	The connect function initiates a connection from the socket with fle descriptor socket
 *	to the socket whose address is specifed by the addr and length arguments. (This
 *	socket is typically on another machine, and it must be already set up as a server.) See
 *	Section 16.3 [Socket Addresses], page 391, for information about how these arguments
 *	are interpreted.
 *	Normally, connect waits until the server responds to the request before it returns.
 *	You can set nonblocking mode on the socket socket to make connect return immedi
 *	ately without waiting for the response. See Section 13.14 [File Status Flags], page 333,
 ...
 *
 *	EBADF The socket socket is not a valid fle descriptor.
 *	ENOTSOCK File descriptor socket is not a socket.
 *	EADDRNOTAVAIL
 *	The specifed address is not available on the remote machine.
 *	EAFNOSUPPORT
 ...
 *	established with select; see Section 13.8 [Waiting for Input or Output],
 *	page 313. Another connect call on the same socket, before the connection
