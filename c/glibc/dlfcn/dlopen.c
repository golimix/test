/* Copyright (C) Rong Tao @Sylincom Beijing, 2019年 06月 14日 星期五 09:05:10 CST. */
/* 
 Copyright (C) Rong Tao @Beijing

 Permission is granted to copy, distribute and/or modify this document
 under the terms of the GNU Free Documentation License, Version 1.3
 or any later version published by the Free Software Foundation;
 with no Invariant Sections, no Front-Cover Texts, and no Back-Cover
 Texts. A copy of the license is included in the section entitled ‘‘GNU
 Free Documentation License’’.
   2019年 03月 14日 星期四 19:24:23 CST. 
*/
/* Copyright (C) Rong Tao @Sylincom Beijing, 2019年 03月 14日 星期四 17:04:49 CST. */
#include <stdio.h>
#include <dlfcn.h>
#include <stdlib.h>


#include "lib/plug.h"

#define PLUG_NAME "lib/libplug.so"

#define _toString(v) #v

int main()
{
	void *dp = NULL;
	char *err = NULL;
	
	int (*plug_open)();
	int (*plug_read)();
	int (*plug_write)();
	int (*plug_close)();

	dp = dlopen(PLUG_NAME, RTLD_LAZY);
	if(!dp)
	{
		printf("%s\n", dlerror());
		exit(1);
	}

	plug_open = dlsym(dp, _toString(plug_open));
	plug_open();

	plug_read = dlsym(dp, _toString(plug_read));
	plug_read();

	dlclose(dp);
	return 0;
}
