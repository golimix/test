 ...
 *	The encrypt function encrypts block if edﬂag is 0, otherwise it decrypts block, using
 *	a key previously set by setkey. The result is placed in block.
 *	Like setkey, block is specifed as an array of 64 bits each stored in a char, but there
 *	are no parity bits in block.
 */
