...
/**
 *	读写锁属性
 *	When initializing a read-write lock, if attr is a null pointer, the default attributes are
 *	used. To assign other than these defaults, the following two functions are provided:
 */
int pthread_rwlockattr_init(pthread_rwlockattr_t *attr);
int pthread_rwlockattr_destroy(pthread_rwlockattr_t *attr);
	//Both return: 0 if OK. ~ositive~ xxvxa lue on error

/**
 *	Once an attribute object of datatype pthread-rwlockattr-t has been initialized,
 *	separate functions are called to enable or disable certain attributes. The only attribute
 *	currently defined is PTHREAD-PROCESS-SHARED, which specifies that the read-write
 *	lock is to be shared between different processes, not just between different threads
 *	within a single process. The following two functions fetch and set this attribute:
 */
