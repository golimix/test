 ...
 *	This expands to a integer constant expression that is the oﬀset of the structure 
 *	member named member in the structure type type. For example, offsetof (struct s,
 *	elem) is the oﬀset, in bytes, of the member elem in a struct s.
 *	This macro won’t work if member is a bit feld; you get an error from the C compiler
 *	in that case.
 */
