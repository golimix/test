...
#define MP_DEBUG(x, args...) do { if (mp_debug) fprintf(stdout, "[MP][D]"x, ##args); fflush (stdout); }while(0)
...
typedef struct memory_pools_s {/*  <rongtao comment: 2019.09.25>*/
  pools_start_mark_t          start_mark;

  uint32_t                    pools_number;
  uint32_t                    pools_defined;
  memory_pool_t              *pools;
} memory_pools_t;/*  <rongtao comment: 2019.09.25>*/
