//以下的示例演示了采用 Engine 机制，来改变openssl 的各种运算行为。实现的Engine
//方法有：随机数方法、对称算法、摘要算法以及RSA 运算算法。其中，RSA 计算中，密钥
//ID 存放在Engine 的扩展数据结构中。
#include <string.h>
            ...
            unsigned char *sigret, unsigned int *siglen, void *key)
{
    ...
    k=(RSA *)key;
    /* 获取硬件中的私钥ID，进行计算 */
    keyid=ENGINE_get_ex_data(k->engine,0);//获取/设置Engine 的扩展数据。
    printf("call mySign\n");
