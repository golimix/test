...
void requestdoc(int flag, const char **doc)
{
        switch(flag) {
        case 1:
                if (xargc == 1 && isatty(fileno(stdin))) pagedoc(doc);
        break;
        case 0:
                if (xargc == 1 && isatty(fileno(stdin)) && isatty(fileno(stdout))) pagedoc(doc);
        break;
        default:
                if (xargc <= flag) pagedoc(doc);
        break;
        }
        return;
}
...
void pagedoc(const char **doc)
{
    if(doc == NULL)
    {
        fprintf(stderr, "no doc to see.\n");
        return;
