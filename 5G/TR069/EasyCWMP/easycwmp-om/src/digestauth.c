...
//  "h5ffku7rlxp6tjf2xamnfqjev5ul" 初始代码中的
//  "VHVlIE5vdiAyOCAxNDo0Nzo1MCAyMDA2MS4xLjEuMTI1" ACS模拟器的随机数 rongtao@sylincom.com
...
void http_digest_init_nonce_priv_key(void) {
    ...
    cwmp_log(NAME, L_NOTICE, "KEY NONCE: %s\n", NONCE_PRIV_KEY);
}
		...
		const char *url, const char *realm, const char *opaque)
{
			...
			realm, nonce, opaque);
	{
		...
		cwmp_log(NAME, L_DEBUG, "%s: header: %s", __FUNCTION__, header);
		...
		const char *password, unsigned int nonce_timeout)
{
	...
	cwmp_log(NAME, L_DEBUG, "%s: header: %s", __FUNCTION__, header);
	cwmp_log(NAME, L_NOTICE, "header: %s", header);
...

	{
        ...
        cwmp_log(NAME, L_NOTICE, "username %s, un %s", username, un);
...

	{
		...
		{
			cwmp_log(NAME, L_DEBUG, "Authentication failed: URI does not match.");
		...
		if ((0 == lookup_sub_value(cnonce, sizeof(cnonce), header, "cnonce"))
				...
				|| (0
						== lookup_sub_value(response, sizeof(response), header,
								"response")))
		{
			cwmp_log(NAME, L_DEBUG, "Authentication failed, invalid format.");
			return MHD_NO;
		}
