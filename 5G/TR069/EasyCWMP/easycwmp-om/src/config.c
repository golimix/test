...
static int config_init_local(void)
{
	...
	uci_foreach_element(&uci_easycwmp->sections, e1) {
		...
		if (strcmp(s->type, "local") == 0) {
			...
			config->local->cr_auth_type = DEFAULT_CR_AUTH_TYPE; //默认为摘要认证
			uci_foreach_element(&s->options, e1) {
				...
				if (!strcmp((uci_to_option(e1))->e.name, "logging_level")) {
					...
					cwmp_log(NAME, L_DEBUG, "easycwmp.@local[0].logging_level=%d\n", config->local->logging_level);
					continue;
				}
				...
				if (!strcmp((uci_to_option(e1))->e.name, "authentication")) {
					...
					cwmp_log(NAME, L_DEBUG, "easycwmp.@local[0].authentication=%s\n",
			...
			if (!config->local->interface) {
				cwmp_log(NAME, L_DEBUG, "in local you must define interface\n");
				return -1;
			}

			if (!config->local->port) {
				cwmp_log(NAME, L_DEBUG, "in local you must define port\n");
				return -1;
			}

			if (!config->local->ubus_socket) {
				cwmp_log(NAME, L_DEBUG, "in local you must define ubus_socket\n");
				return -1;
			}
	...
	cwmp_log(NAME, L_DEBUG, "uci section local not found...\n");
	return -1;
}
...
static int config_init_acs(void)
{
		...
		if (strcmp(s->type, "acs") == 0) {
			...
			uci_foreach_element(&s->options, e) {
				...
				if (!strcmp((uci_to_option(e))->e.name, "periodic_time")) {
					...
					cwmp_log(NAME, L_DEBUG, "easycwmp.@acs[0].periodic_time=%s\n", uci_to_option(e)->v.string);
					continue;
				}
				...
				if (!strcmp((uci_to_option(e))->e.name, "ssl_verify")) {
					...
					cwmp_log(NAME, L_DEBUG, "easycwmp.@acs[0].ssl_verify=%d\n", config->acs->ssl_verify);
					continue;
				}
			}

			if (!config->acs->url) {
				cwmp_log(NAME, L_DEBUG, "acs url must be defined in the config\n");
				return -1;
			}
	...
	cwmp_log(NAME, L_DEBUG, "uci section acs not found...\n");
	return -1;
}
...
static int config_init_device(void)
{
		...
		if (strcmp(s->type, "device") == 0) {
			...
			uci_foreach_element(&s->options, e2) {
				if (!strcmp((uci_to_option(e2))->e.name, "software_version")) {
					...
					cwmp_log(NAME, L_DEBUG, "easycwmp.@device[0].software_version=%s\n", config->device->software_version);
					continue;
				}
			}
			return 0;
	...
	cwmp_log(NAME, L_DEBUG, "uci section device not found...\n");
	return -1;
}

static struct uci_package *config_init_package(const char *c)
{
    /* 分配内存 */
	if (first_run) {
...
void easycwmp_config_exit(void)
{
	if (config) {
		config_free_acs();
		FREE(config->acs);
...
void config_load(void)
{
    ...
    cwmp_log(NAME, L_CRIT, "call config_load done\n");
    
	return;

error:
	cwmp_log(NAME, L_CRIT, "configuration (re)loading failed, exit daemon\n");
	exit(EXIT_FAILURE);
}
