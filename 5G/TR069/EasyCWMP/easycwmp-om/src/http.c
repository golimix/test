...
http_client_init(void)
{
	...
	cwmp_log(NAME, L_DEBUG, "+++ HTTP CLIENT CONFIGURATION +++\n");
	cwmp_log(NAME, L_DEBUG, "url: %s\n", http_c.url);
	if (config->acs->ssl_cert)
		cwmp_log(NAME, L_DEBUG, "ssl_cert: %s\n", config->acs->ssl_cert);
	if (config->acs->ssl_cacert)
		cwmp_log(NAME, L_DEBUG, "ssl_cacert: %s\n", config->acs->ssl_cacert);
	if (!config->acs->ssl_verify)
		cwmp_log(NAME, L_DEBUG, "ssl_verify: SSL certificate validation disabled.\n");
	cwmp_log(NAME, L_DEBUG, "--- HTTP CLIENT CONFIGURATION ---\n");
...
# endif /* DEVEL */
	...
	cwmp_log(NAME, L_NOTICE, "configured acs url %s\n", http_c.url);
	return 0;
}
...
void http_client_exit(void)
{
	...
	if(remove(fc_cookies) < 0)
		cwmp_log(NAME, L_NOTICE, "can't remove file %s\n", fc_cookies);
}
...
int8_t http_send_message(char *msg_out, char **msg_in)
{
	...
	if (msg_out) {
		cwmp_log(NAME, L_DEBUG, "+++ SEND HTTP REQUEST +++\n%s\n", msg_out);
		cwmp_log(NAME, L_DEBUG, "--- SEND HTTP REQUEST ---\n");
		...
		cwmp_log(NAME, L_DEBUG, "+++ SEND EMPTY HTTP REQUEST +++\n");
	...
	*msg_in = (char *) calloc (1, sizeof(char));
	...
	if (error_buf[0] != '\0')
		cwmp_log(NAME, L_NOTICE, "LibCurl Error: %s\n", error_buf);
        //libcurl error: Empty reply from server
	if (!strlen(*msg_in)) {
		FREE(*msg_in);
	}
	...
	if (httpCode == 302 || httpCode == 307) {
		...
		http_client_exit();
		if (http_client_init()) {
			cwmp_log(NAME, L_DEBUG, "receiving http redirect: re-initializing http client failed\n");
	...
	if (res || (httpCode != 200 && httpCode != 204)) {
		cwmp_log(NAME, L_NOTICE, "sending http message failed\n");
		return -1;
	}

	if (*msg_in) {
		cwmp_log(NAME, L_DEBUG, "+++ RECEIVED HTTP RESPONSE +++\n%s\n", *msg_in);
		cwmp_log(NAME, L_DEBUG, "--- RECEIVED HTTP RESPONSE ---\n");
	} else {
		cwmp_log(NAME, L_DEBUG, "+++ RECEIVED EMPTY HTTP RESPONSE +++\n");
	}
...
void http_server_init(void)
{
	...
	cwmp_log(NAME, L_DEBUG, "+++ HTTP SERVER CONFIGURATION +++\n");
	if (config->local->ip)
		cwmp_log(NAME, L_DEBUG, "ip: '%s'\n", config->local->ip);
	else
		cwmp_log(NAME, L_DEBUG, "NOT BOUND TO IP\n");
	cwmp_log(NAME, L_DEBUG, "port: '%s'\n", config->local->port);
	cwmp_log(NAME, L_DEBUG, "--- HTTP SERVER CONFIGURATION ---\n");

	cwmp_log(NAME, L_NOTICE, "http server initialized\n");
}

static void http_new_client(struct uloop_fd *ufd, unsigned events)
{
	struct timeval t;
	int cr_auth_type = config->local->cr_auth_type; //正数认证类型？
	char buffer[BUFSIZ];
	...
	for (;;) {
		...
		if (setsockopt(ufd->fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&t, sizeof t)) {
			cwmp_log(NAME, L_DEBUG, "setsockopt() failed\n");
		}
		if (client < 0) {
			break;
		}
		...
		cwmp_log(NAME, L_DEBUG, "+++ RECEIVED HTTP REQUEST +++\n");
		*buffer = '\0';
		while (fgets(buffer, sizeof(buffer), fp)) {
			...
			if (!username || !password) {
				...
				if (http_digest_auth_check("GET", "/", auth_digest + strlen("Authorization: Digest "), REALM, username, password, 300) == MHD_YES)
					...
					auth_status = 0;
					cwmp_log(NAME, L_NOTICE, "Connection Request authorization failed\n");
                    cwmp_log(NAME, L_NOTICE, "username %s, password %s\n",  username, password);
				}
			}
...
http_end:
		if (*buffer) {
			...
			if (auth_status) {
				...
				cwmp_log(NAME, L_DEBUG, "+++ HTTP SERVER CONNECTION SUCCESS +++\n");
				cwmp_log(NAME, L_NOTICE, "ACS initiated connection\n");
