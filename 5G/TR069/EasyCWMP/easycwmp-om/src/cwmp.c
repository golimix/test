...
static inline void cwmp_retry_session() {
	...
	cwmp_log(NAME, L_NOTICE, "retry session in %d sec, RetryCount = %d\n", rtime, cwmp->retry_count);
...
static inline int rpc_transfer_complete(mxml_node_t *node, int *method_id)
{
	...
	if ( backup_extract_transfer_complete(node, &msg_out, method_id)) {
		cwmp_log(NAME, L_DEBUG, " Transfer Complete xml message creating failed\n");
		return -1;
	}

	cwmp_log(NAME, L_NOTICE, "send RPC ACS TransferComplete\n");

	do {
		FREE(msg_in);

		if (http_send_message(msg_out, &msg_in)) {
			cwmp_log(NAME, L_DEBUG, "sending Transfer Complete http message failed\n");
			error = -1; break;
		}
		if (!msg_in)
			break;
		...
		if (error == -1) {
			cwmp_log(NAME, L_DEBUG, "parse Transfer Complete xml message from ACS failed\n");
			break;
		}
...
static inline int rpc_get_rpc_methods()
{
	...
	if (xml_prepare_get_rpc_methods_message(&msg_out)) {
		cwmp_log(NAME, L_DEBUG, "GetRPCMethods xml message creating failed\n");
		return -1;
	}

	cwmp_log(NAME, L_NOTICE, "send RPC ACS GetRPCMethods\n");

	do {
		FREE(msg_in);

		if (http_send_message(msg_out, &msg_in)) {
			cwmp_log(NAME, L_DEBUG, "sending GetRPCMethods http message failed\n");
			error = -1; break;
		}
		...
		if (error == -1) {
			cwmp_log(NAME, L_DEBUG, "parse GetRPCMethods xml message from ACS failed\n");
			break;
		}
		else if (error && (error != FAULT_ACS_8005)) {
			error = 0; break;
...
static inline int rpc_inform()
{
    ...
    cwmp_log(NAME, L_NOTICE, ">> xml_prepare_inform_message \n");

    /* 准备发送给ACS的XML, inform消息 */
	if (xml_prepare_inform_message(&msg_out)) {
		cwmp_log(NAME, L_DEBUG, "Inform xml message creating failed\n");
		return -1;
	}

	cwmp_log(NAME, L_NOTICE, ">> send Inform\n");

	do {
		FREE(msg_in);

        /* 发送Http消息：Inform， HTTP Post */
		if (http_send_message(msg_out, &msg_in)) {
            cwmp_log(NAME, L_NOTICE, ">> sending Inform http message failed \n");
			cwmp_log(NAME, L_WARNING, "sending Inform http message failed\n");
			error = -1; break;
		}
        cwmp_log(NAME, L_NOTICE, ">> sending Inform http message success \n");
        
		if (!msg_in) {
			cwmp_log(NAME, L_WARNING, "parse Inform xml message from ACS: Empty message\n");
			error = -1; break;
		}

		error = xml_parse_inform_response_message(msg_in);
		if (error && (error != FAULT_ACS_8005)) {
			cwmp_log(NAME, L_WARNING, "parse Inform xml message from ACS failed\n");
			error = -1; break;
		}
        
        cwmp_log(NAME, L_NOTICE, ">> count %d, error %d \n", count, error);
	} while(error && (count++)<10);
...
int cwmp_inform(void)
{
	...
	cwmp_log(NAME, L_NOTICE, "start session\n");
	if (http_client_init()) {
		cwmp_log(NAME, L_DEBUG, "initializing http client failed\n");
		goto error;
	}
	if (external_init()) {
		cwmp_log(NAME, L_DEBUG, "external scripts initialization failed\n");
		goto error;
	}
    cwmp_log(NAME, L_NOTICE, ">> call rpc_inform \n");
	if(rpc_inform()) {
		cwmp_log(NAME, L_NOTICE, "sending Inform failed\n");
		goto error;
	}
	cwmp_log(NAME, L_NOTICE, "receive InformResponse from the ACS\n");
	...
	do {
		while((node = backup_check_transfer_complete()) && !cwmp->hold_requests) {
			if(rpc_transfer_complete(node, &method_id)) {
				cwmp_log(NAME, L_NOTICE, "sending TransferComplete failed\n");
				goto error;
			}
			cwmp_log(NAME, L_NOTICE, "receive TransferCompleteResponse from the ACS\n");
		...
		if(cwmp->get_rpc_methods && !cwmp->hold_requests) {
			if(rpc_get_rpc_methods()) {
				cwmp_log(NAME, L_NOTICE, "sending GetRPCMethods failed\n");
				goto error;
			}
			cwmp_log(NAME, L_NOTICE, "receive GetRPCMethodsResponse from the ACS\n");

			cwmp->get_rpc_methods = false;
		}

		if (cwmp_handle_messages()) {
			cwmp_log(NAME, L_DEBUG, "handling xml message failed\n");
			goto error;
		}
	...
	external_exit();
	cwmp->retry_count = 0;
	cwmp_log(NAME, L_NOTICE, "end session success\n");
	return 0;
...
error:
	...
	cwmp_handle_end_session();
	external_exit();
	cwmp_log(NAME, L_NOTICE, "end session failed\n");
	cwmp_retry_session();
...
int cwmp_handle_messages(void)
{
	...
	cwmp_log(NAME, L_NOTICE, "send empty message to the ACS\n");

	while (1) {
		FREE(msg_in);

		if (http_send_message(msg_out, &msg_in)) {
			cwmp_log(NAME, L_DEBUG, "sending http message failed\n");
			goto error;
		}

		if (!msg_in) {
			cwmp_log(NAME, L_NOTICE, "receive empty message from the ACS\n");
			break;
		}
		...
		if (xml_handle_message(msg_in, &msg_out)) {
			cwmp_log(NAME, L_NOTICE, "handling message failed\n");
			cwmp_log(NAME, L_DEBUG, "xml handling message failed\n");
			goto error;
		}

		if (!msg_out) {
			cwmp_log(NAME, L_NOTICE, "handling message failed\n");
			cwmp_log(NAME, L_DEBUG, "acs response message is empty\n");
			goto error;
		}
	}
	FREE(msg_in);
...
void cwmp_add_scheduled_inform(char *key, int delay)
{
	...
	s = calloc(1, sizeof(*s));
	if (!s) return;
	cwmp_log(NAME, L_NOTICE, "scheduled inform in %d sec\n", delay);
...
void cwmp_download_launch(struct uloop_timeout *timeout)
{
	...
	cwmp_log(NAME, L_NOTICE, "start download url = %s, FileType = '%s', CommandKey = '%s'\n",
...
end_fault :
	cwmp_log(NAME, L_NOTICE, "download error: '%s'\n", fault_array[code].string);
...
void cwmp_upload_launch(struct uloop_timeout *timeout)
{
	...
	cwmp_log(NAME, L_NOTICE, "start upload url = %s, FileType = '%s', CommandKey = '%s'\n",
...
end_fault :
	cwmp_log(NAME, L_NOTICE, "upload error: '%s'\n", fault_array[code].string);
	backup_update_fault_transfer_complete(node, code);
	cwmp_add_inform_timer();
	external_exit();
	cwmp_free_upload(d);
...
void cwmp_add_download(char *key, int delay, char *file_size, char *download_url, char *file_type, char *username, char *password, mxml_node_t *node)
{
	...
	cwmp_log(NAME, L_NOTICE, "add download: delay = %d sec, url = %s, FileType = '%s', CommandKey = '%s'\n",
...
void cwmp_add_upload(char *key, int delay, char *upload_url, char *file_type, char *username, char *password, mxml_node_t *node)
{
	...
	cwmp_log(NAME, L_NOTICE, "add upload: delay = %d sec, url = %s, FileType = '%s', CommandKey = '%s'\n",
...
struct event *cwmp_add_event(int code, char *key, int method_id, int backup)
{
	...
	cwmp_log(NAME, L_NOTICE, "add event '%s'\n", event_code_array[code].code);
...
{
    cwmp_log(NAME, L_NOTICE, "call cwmp_init_deviceid\n");
    
	external_action_simple_execute("inform", "device_id", NULL);
    
    cwmp_log(NAME, L_NOTICE, "call cwmp_init_deviceid done\n");
    
	if (external_action_handle(json_handle_deviceid))
		return -1;

    cwmp_log(NAME, L_NOTICE, "call external_action_handle done\n");

	if (!cwmp->deviceid.product_class || cwmp->deviceid.product_class[0] == '\0') {
		cwmp_log(NAME, L_NOTICE, "in device you must define product_class\n");
		return -1;
	}

	if (!cwmp->deviceid.serial_number || cwmp->deviceid.serial_number[0] == '\0') {
		cwmp_log(NAME, L_NOTICE, "in device you must define serial_number\n");
		return -1;
	}

	if (!cwmp->deviceid.manufacturer || cwmp->deviceid.manufacturer[0] == '\0') {
		cwmp_log(NAME, L_NOTICE, "in device you must define manufacturer\n");
		return -1;
	}

	if (!cwmp->deviceid.oui || cwmp->deviceid.oui[0] == '\0') {
		cwmp_log(NAME, L_NOTICE, "in device you must define manufacturer oui\n");
		return -1;
	}
    cwmp_log(NAME, L_NOTICE, "call cwmp_init_deviceid done\n");
