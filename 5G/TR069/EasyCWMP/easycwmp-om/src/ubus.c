...
{
	cwmp_log(NAME, L_NOTICE, "triggered ubus notification\n");
			...
			struct blob_attr *msg)
{
	...
	cwmp_log(NAME, L_NOTICE, "triggered ubus inform %s\n",
			 ...
			 struct blob_attr *msg)
{
	...
	if (!strcmp("reload", cmd)) {
		cwmp_log(NAME, L_NOTICE, "triggered ubus reload\n");
		easycwmp_reload();
		blobmsg_add_u32(&b, "status", 0);
		if (asprintf(&info, "easycwmpd reloaded") == -1)
			goto error;
	} else if (!strcmp("stop", cmd)) {
		cwmp_log(NAME, L_NOTICE, "triggered ubus stop\n");
