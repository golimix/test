...
int backup_save_file(void) 
{
    ...
    cwmp_log(NAME, L_NOTICE, ">> easycwmp backup data %s\n", val);
	...
	if (fp!=NULL) {
		...
		if (len > 0 && val[len - 1] == '\n')
			val[len - 1]= '\0';
		str_replace_newline_byspace(val);
//        cwmp_log(NAME, L_NOTICE, ">> backup data %s\n", val);
		fprintf(fp, "%s", val);
		fclose(fp);
		free(val);
		return 0;
...
/* 下载文件 */
mxml_node_t *backup_add_download(char *key, int delay, char *file_size, char *download_url, 
                                   char *file_type, char *username, char *password)
{
