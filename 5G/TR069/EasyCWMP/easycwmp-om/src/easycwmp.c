...
static void easycwmp_do_reload(struct uloop_timeout *timeout)
{
	cwmp_log(NAME, L_NOTICE, "configuration reload\n");
	if (external_init()) {
...
static void easycwmp_do_notify(struct uloop_timeout *timeout)
{
	cwmp_log(NAME, L_NOTICE, "checking if there is notify value change\n");
	if (external_init()) {
		cwmp_log(NAME, L_DEBUG, "external scripts initialization failed\n");
		return;
	}
...
static void easycwmp_netlink_interface(struct nlmsghdr *nlh)
{
	...
	while (rtl && RTA_OK(rth, rtl)) {
		...
		if ((addr != old_addr) && (old_addr != 0)) {
			cwmp_log(NAME, L_NOTICE, "ip address of the interface %s is changed\n",	if_name);
	...
	cwmp_log(NAME, L_NOTICE, "interface %s has ip %s\n",
			if_name, if_addr);
}
...
netlink_new_msg(struct uloop_fd *ufd, unsigned events)
{
	...
	if ((msg_size = recv(ufd->fd, nlh, BUFSIZ, 0)) == -1) {
		cwmp_log(NAME, L_DEBUG, "error receiving netlink message\n");
		return;
	}
	...
	while (msg_size > sizeof(*nlh)) {
		...
		if (req_len < 0 || len > msg_size) {
			cwmp_log(NAME, L_DEBUG, "error reading netlink message\n");
			return;
		}

		if (!NLMSG_OK(nlh, msg_size)) {
			cwmp_log(NAME, L_DEBUG, "netlink message is not NLMSG_OK\n");
			return;
		}
...
static int netlink_init(void)
{
	...
	if ((send(cwmp->netlink_sock[1], &req, req.hdr.nlmsg_len, 0)) == -1) {
		cwmp_log(NAME, L_DEBUG, "couldn't send netlink socket");
		return -1;
	}
...
int tr069_cwmp_init()
{
    ...
    const static int argc = 3;
    const char *argv[] = {
        "easycwmpd", "-f", "-b"
            };

    return easycwmpd_init(argc, argv);        
}

int easycwmpd_init(int argc, char **argv)
{
	int c;
	int start_event = 0;
	...
	if (getuid() != 0) {
		cwmp_log(NAME, L_DEBUG, "run %s as root\n", NAME);
		exit(EXIT_FAILURE);
	}
	...
	config_load();
	cwmp_log(NAME, L_NOTICE, "daemon started\n");
    ...
    cwmp_log(NAME, L_NOTICE, " start_event >> \n");
    
	if (start_event & START_BOOT) {
        cwmp_log(NAME, L_NOTICE, " START_BOOT\n");
		cwmp_add_event(EVENT_BOOT, NULL, 0, EVENT_BACKUP);
		cwmp_add_inform_timer();
        cwmp_log(NAME, L_NOTICE, " call cwmp_add_inform_timer\n");
	}
	if (start_event & START_GET_RPC_METHOD) {
        cwmp_log(NAME, L_NOTICE, " START_GET_RPC_METHOD\n");
	...
	if (netlink_init()) {
		cwmp_log(NAME, L_DEBUG, "netlink initialization failed\n");
		exit(EXIT_FAILURE);
	}

	if (ubus_init()) cwmp_log(NAME, L_DEBUG, "ubus initialization failed\n");
	...
	if (!foreground) {
		...
		sid = setsid();
		if (sid < 0) {
			cwmp_log(NAME, L_DEBUG, "setsid() returned error\n");
			exit(EXIT_FAILURE);
		}
		...
		if ((chdir(directory)) < 0) {
			cwmp_log(NAME, L_DEBUG, "chdir() returned error\n");
			exit(EXIT_FAILURE);
		}
	}
	char *buf = NULL;
	if (asprintf(&buf, "%d", getpid()) != -1) {
		...
		if ( error < 0) {
			cwmp_log(NAME, L_DEBUG, "Unable to write the easycwmpd pid to /var/run/easycwmpd.pid\n");
		}

		free(buf);
	}

	cwmp_log(NAME, L_NOTICE, "entering main loop\n");
	uloop_run();

    cwmp_log(NAME, L_NOTICE, ">> ubus_exitn");
	ubus_exit();
	uloop_done();

    cwmp_log(NAME, L_NOTICE, ">> http_client_exit");
	http_client_exit();
	xml_exit();
	...
	cwmp_log(NAME, L_NOTICE, "exiting\n");
	return 0;
}
