...
typedef struct GetParameterNamesResponse_s {
    ...
    ParameterInfoStruct ParameterList[0];
...
typedef struct GetParameterValuesResponse_s {
    ParameterValueStruct Parameter;
}GetParameterValuesResponse;

typedef struct SetParameterAttributesStruct_s {
    char Name[256];
    bool NotificationChange;//If true, the value of Notification replaces the current notification setting for this Parameter or group of Parameters. If false, no change is made to the notification setting.
    int Notification;   //Indicates whether (and how) the CPE will notify the ACS when the specified Parameter(s) change in value. The following values are defined:
...
/**
 *  函数名称：注册TR069 RPC GetParameterNames 响应函数
 *  参数：
 *  @param:get_response 当CWMP收到来自 ACS的 GetParameterNames 时，该函数被调用。
 *                      该函数中必须满足非阻塞条件，可设计成同步消息机制（参见SNMP get代码实现）
 *              request: 函数入参
 *              response: 函数出参
 */
...
/**
 *  函数名称：注册TR069 CWMP初始化函数
 */
int tr069_cwmp_init();


typedef int (*tr069_oid_GetParamValue)(ParameterValueStruct *paramValue);
typedef int (*tr069_oid_SetParamValue)(const ParameterValueStruct *paramValue);
typedef int (*tr069_oid_GetParamAttribute)(SetParameterAttributesStruct *paramAttribute);
typedef int (*tr069_oid_SetParamAttribute)(const SetParameterAttributesStruct *paramAttribute);
...
#ifdef ________rongtao_test_tr069
