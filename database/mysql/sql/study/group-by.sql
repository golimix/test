...
select name,count(*) as count_price from goods group by name;



use custome;

select customer.customer_id, item.item_id from customer right outer join item on customer.customer_id = item.item_id group by item.item_id;
