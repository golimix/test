...
select * from product where id in(1,2) order by name;
select * from product where id in(3,3) order by name;

select * from product where id not in(1,2) order by name;

select * from goods where number in (989,99); --只是两个数的匹配，不是范围

select * from goods where number in (select number from goods where name = 'books')

select * from goods where number in (select number from goods where name = (select name from goods where number in (100)));
