...
/**
 *	Process Environment
 *	The environ entry contains a process's environment (see Section 2.1.6, "The Environment" ). As
 *	with cmdline, the individual environment variables are separated by NULs. The format of each
 *	element is the same as that used in the environ variable, namely VARIABLE=value .
 *	Listing 7.4 presents a generalization of the program in Listing 2.3 in Section 2.1.6. This version
 *	takes a process ID number on its command line and prints the environment for that process by
