...
//	memset(g_tcp_client_socket, 0, MAX_TCP_CLIENT * sizeof(TcpClientSocket));
	...
	for (i = 0; i < BUFF_SIZE; i++) {
		tcp_buff[i] = (UINT8 *)sbs_malloc(MAX_TCP_DATA_LEN);
		if (NULL == tcp_buff[i]) {
		...
		udp_buff[i] = (UINT8 *)sbs_malloc(MAX_UDP_DATA_LEN);
		if (NULL == udp_buff[i]) {
			perror("init socket malloc:");
			return Return_FAIL;
		}
...
//	memset(&g_udp_socket[current_udp_num].send_addr, 0, sizeof(struct sockaddr_in));
	...
	g_udp_socket[current_udp_num].send_addr.sin_addr.s_addr = inet_addr((char*)send_addr);//target address.
	...
	current_udp_num = (current_udp_num + 1) % MAX_UDP_NUM;
	log_msg(LOG_INFO, SOCKET_COMMON, "Success to create udp socket, which send addr=%s, send port=%d, "
...
INT32 alter_udp_addr(INT32 socket_index, UINT8 *send_addr, INT32 send_port)
{
	if (socket_index < -1 || socket_index >= MAX_UDP_NUM) {
		log_msg(LOG_ERR, SOCKET_COMMON, "socket_index error!\n");
		return Return_FAIL;
	}

	if (g_udp_socket[socket_index].socket_conn == 0) {
		log_msg(LOG_ERR, SOCKET_COMMON, "Has no this udp socket.\n");
		return Return_FAIL;
	}
	g_udp_socket[socket_index].send_addr.sin_family = AF_INET;
	g_udp_socket[socket_index].send_addr.sin_addr.s_addr = inet_addr((char*)send_addr);//target address.
...
INT32 create_tcp_socket(UINT8 *server_addr, INT32 server_port, SOCK_FUNC rx_deal_func, SocketRxType rx_type, TcpType tcp_type)
{
	...
	server_sock_addr.sin_addr.s_addr = inet_addr((char*)server_addr);
	...
	if (TCP_CLIENT == tcp_type) {
		...
		log_msg(LOG_SUMMARY, SOCKET_COMMON, "Have connect a client server!\n");
		...
		log_msg(LOG_SUMMARY, SOCKET_COMMON,"Henb listening... addr=%s,port=%d\n",server_addr,server_port);
...
INT32 close_tcp_server_socket(INT32 tcp_index, INT32 socket_index)
{
	...
	if (tcp_index < -1 || tcp_index >= MAX_TCP_SERVER) {
		log_msg(LOG_ERR, SOCKET_COMMON, "TCP server index error.\n");
		return Return_FAIL;
	}
	if (socket_index < -1 || socket_index >= MAX_TCP_USER) {
		log_msg(LOG_ERR, SOCKET_COMMON, "TCP server socket_index error.\n");
		return Return_FAIL;
	}
		...
		if (0 != g_tcp_server_socket[tcp_index].server_socket) {
			if (socket_index == -1) { //delete all of a tcp server's clients
				...
				if (0 != g_tcp_server_socket[tcp_index].client_socket[socket_index]) {
					...
					log_msg(LOG_ERR, SOCKET_COMMON, "Has no this tcp socket.\n");
					return Return_FAIL;
				}
			}
		} else {
...
INT32 close_tcp_client_socket(INT32 socket_index)
{
	...
	if (socket_index < -1 || socket_index >= MAX_TCP_CLIENT){
		log_msg(LOG_ERR, SOCKET_COMMON, "TCP client socket_index error.\n");
		return Return_FAIL;
	}
	...
	if (socket_index == -1) {
		...
		if (0 != g_tcp_client_socket[socket_index].server_socket) {
			...
			log_msg(LOG_ERR, SOCKET_COMMON, "Has no this tcp client socket.\n");
			return Return_FAIL;
		}
	}
...
INT32 tx_udp_msg(INT32 socket_index, UINT8 *msg_p, INT32 msg_len)
{
	if (socket_index < 0 || socket_index >= MAX_UDP_NUM){
		log_msg(LOG_ERR, SOCKET_COMMON, "UDP socket_index=%d error.\n",socket_index);
		return Return_FAIL;
	}
...
INT32 tx_ul_data2socket(INT32 socket, UINT32 port, UINT8 *msg_p, INT32 msg_len)
{
	...
	if (socket < 0){
		log_msg(LOG_ERR, SOCKET_COMMON, "UDP socket =%d error.\n",socket);
		return Return_FAIL;
	}
...
INT32 rx_udp_msg(INT32 socket_index, UINT8 **msg_p_p, INT32 *msg_len_p)
{
	...
	INT32 result = 0;
	if (socket_index < 0 || socket_index >= MAX_UDP_NUM){
		log_msg(LOG_ERR, SOCKET_COMMON, "UDP socket_index error.\n");
		return Return_FAIL;
	}
	...
	result = recvfrom(g_udp_socket[socket_index].socket_conn, udp_buff[udp_buff_num], MAX_UDP_DATA_LEN,
						0, NULL,  NULL);
	if (result > 0) {
		*msg_p_p = (UINT8 *)sbs_malloc(result * sizeof(UINT8));
		memcpy(*msg_p_p, udp_buff[udp_buff_num], result);
		//(*msg_p_p)[result]='\0';
		*msg_len_p = result;
		udp_buff_num = (udp_buff_num + 1) % BUFF_SIZE;
	} else {
		log_msg(LOG_ERR, SOCKET_COMMON, "UDP receive error.udp socket_index=%d\n", socket_index);
...
inline INT32 rx_udp_buff(INT32 socket_fd, UINT8 *buff_p)
{
	...
	if (result > 0) {
		...
		log_msg(LOG_SUMMARY, SOCKET_COMMON, "close udp socket[%d]\n",socket_fd);
	}
	return Return_FAIL;
}
...
INT32 rx_tcp_server_msg(INT32 tcp_index, INT32 socket_index, UINT8 **msg_p_p, INT32 *msg_len_p)
{
	...
	while (1){
		if (recv_flag == 0){
			...
			if (ret <= 0){
            	log_msg(LOG_ERR, SOCKET_COMMON, "The tcp receive data error .\n");
            	return Return_FAIL;
       		}
			*msg_len_p = recv_len;
			if (recv_len > MAX_TCP_DATA_LEN) {
				log_msg(LOG_ERR, SOCKET_COMMON, "The tcp receive data size has expired the max tcp data size .\n");
				return Return_FAIL;
			} else if (recv_len <= 0) {
				log_msg(LOG_ERR, SOCKET_COMMON, "The tcp receive data error .\n");
				return Return_FAIL;
			}
			recv_buff = (UINT8 *)sbs_malloc(recv_len);
			recv_flag = 1;
		}
				...
				if (FD_ISSET(g_tcp_server_socket[i].server_socket, &ready_set)) {
					...
					while (g_tcp_server_socket[i].client_socket[g_tcp_server_socket[i].current_num] != 0) {
						...
						if (g_tcp_server_socket[i].current_num == user_index) {
							log_msg(LOG_ERR, SOCKET_COMMON, "Has reached the max client number.\n");
							user_flag = 0;
							break;
						}
					}
					if (user_flag == 1) {
						...
						if (-1 == g_tcp_server_socket[i].client_socket[user_index]) {
							log_msg(LOG_ERR, SOCKET_COMMON, "Error accetp UE connect:%s", strerror(errno));
							exit(1);
						}else{
							log_msg(LOG_SUMMARY, SOCKET_COMMON,"Welcome ue[%s] idx:%d connecting to HeNB\n",
				...
				for (j = 0; j < MAX_TCP_USER; j++) {
					if (g_tcp_server_socket[i].client_socket[j]!= 0 && FD_ISSET(g_tcp_server_socket[i].client_socket[j], &ready_set)) {
						...
						if (ret != 0) {
							...
							log_msg(LOG_SUMMARY, SOCKET_COMMON, "close tcp server client socket[%d][%d]\n",i,j);
							continue;
						} // if(rece_len)
			...
			for (i = 0; i < MAX_TCP_CLIENT; i++) {
				if (g_tcp_client_socket[i].server_socket !=0 && g_tcp_client_socket[i].rx_type == SOCKET_TOGETHER &&
					...
					if (ret != 0) {
						...
						log_msg(LOG_SUMMARY, SOCKET_COMMON, "close tcp client socket[%d]\n",i);
						continue;
					} // if(rece_len)
