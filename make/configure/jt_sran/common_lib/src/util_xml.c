                          ...
                          const char **argv)
{
    ...
    if(doc == NULL)
    {
    	log_msg(LOG_ERR,MAIN,"Document not parsed successfully!<%s>\n",docname);
        return NULL;
    }
    cur = xmlDocGetRootElement(doc);
    if(cur == NULL)
    {
    	log_msg(LOG_ERR,MAIN,"Empty document!<%s>\n",docname);
        xmlFreeDoc(doc);
        return NULL;
    }
    ...
    {
    	log_msg(LOG_ERR,MAIN,"Docuument of the wrong type!<%s>\n",cur->name);
        xmlFreeDoc(doc);
        return NULL;
    }
...
xmlDocPtr cfg_parse(char *szDocName)
{
    ...
    if (doc == NULL)
    {
    	log_msg(LOG_ERR,MAIN,"doc is null\n");
        return NULL;
    }
    return doc;
}
...
xmlXPathContextPtr cfg_context(xmlDocPtr doc)
{
    ...
    if (context ==  NULL)
    {
    	log_msg(LOG_ERR, MAIN, "context is null\n");
        return NULL;
    }
    return context;
}
                            ...
                            const xmlChar *content)
{
    ...
    if (result == NULL)
    {
    	log_msg(LOG_ERR, MAIN, "result is null\n");
        return -1;
    }
    if (xmlXPathNodeSetIsEmpty(result->nodesetval))
    {
        xmlXPathFreeObject(result);
        log_msg(LOG_ERR, MAIN,"nodeset:%s is empty\n",xpath);
        return -1;
    }
...
xmlChar* get_cfg_node_value(xmlXPathContextPtr context, const xmlChar *xpath)
{
    ...
    {
        xmlXPathFreeObject(result);
        log_msg(LOG_ERR, MAIN, "nodeset is empty\n");
        return NULL;
    }
