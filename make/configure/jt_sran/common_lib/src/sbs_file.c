...
INT32 get_file_len(FILE *file_p)/*note the current the pos ,the set the file stream to end,finally recovery previous pos */
{
	fpos_t cur_pos;
	INT32 len;
	...
	len = tell_file(file_p);/*current position to compute the location*/
 ...
 *         return Return_ALL_RIGHT, if success.
 *                EOF, if error ocuurs.
 ******************************************************************************/
INT32 getpos_file(FILE *file_p, fpos_t *pos)
{
	return fgetpos(file_p, pos); /*get the position of current position*/
}
 ...
 *
 * Output: return Return_ALL_RIGHT, if success.
 *                EOF, if error ocuurs.
 ******************************************************************************/
