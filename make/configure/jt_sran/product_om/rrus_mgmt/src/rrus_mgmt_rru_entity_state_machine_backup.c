...
#include "rrus_mgmt_common_ctrl_msg_handle.h"
#include "rrus_mgmt_monitor_msg_handle.h"
...
//    }
    ...
    add_rru_table_error:
		log_msg(LOG_ERR, VBS_MGMT, "add state table entry err!\n");
        return -1;
}
...
INT32 exec_rru_sm(UINT32 rru_id, RruEventType event, void *msg)
{
	...
	if (NULL == msg) 
	{
		log_msg(LOG_ERR, VBS_MGMT, "message pointer is NULL.\n");
    ...
    {
        log_msg(LOG_ERR, MAIN, "Cannot find rru_id = %d in rru_entity_table.\n", rru_id);
        return -1;
    }
	...
	{
		log_msg(LOG_ERR, MAIN, "rru state is not in range!\n");
	...
	{
        #ifdef DEBUG
		log_msg(LOG_INFO, MAIN, "Receive RRU Event! (rru_id = %d, state = %s, event "
		   			"= %s)\n", rru_id, rru_state_name[rru_state], rru_event_name[event]);
        #endif
	} 
	else 
	{
		log_msg(LOG_ERR, MAIN, "p_handle_fn is NULL pointer! (rru_id = %d, state = %s, event "
