...
#else
    if (skb->dst)/* no dst content  [rongtao comment 2019.10.17]*/
    {
        dst_release(skb->dst);
        skb->dst = 0UL;
    }
...
#if (KVERSION > 18)
static unsigned int
xt_gtpu_target(struct sk_buff *skb, const struct xt_target_param *par)
{
...
{
    int result = NF_ACCEPT;
    const struct xt_gtpu_target_info *tgi = targinfo;
    ...
    if (tgi->action == PARAM_GTPU_ACTION_ADD)
    {
        result = _gtpu_target_add(*pskb, tgi);
    }
    else if (tgi->action == PARAM_GTPU_ACTION_REM)
    {
        result = _gtpu_target_rem(*pskb, tgi);
    }
    else if (tgi->action == PARAM_GTPU_ACTION_TRANSPORT)
    {
    }
