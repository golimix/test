...
void icmp_sig_handler(int signo)
{
    switch(signo)
    {
        ...
        case SIGINT:
            ICMP_LOG("\n--- ping statistics ---\n\r");
            ICMP_LOG("%d packets transmitted, %d received, %d%% lost\n\r", 
...
int icmp_dst_addr(const char *addrHost, struct sockaddr_in * dst_addr)
{
    ...
    ICMP_LOG("PING %s(%s): %d bytes data in ICMP packets.\n\r", 
            addrHost, inet_ntoa(dst_addr->sin_addr), ICMP_PKG_SIZE);
...
int icmp_pkg_unpack(struct sockaddr_in * addr, char * buf, int len)
{
    ...
    if ((icmp->icmp_type == ICMP_ECHOREPLY) && (icmp->icmp_id = ntohs(ICMP_MAGIC_ID)))
    {
        ...
        ICMP_LOG("%d byte from %s: icmp_req=%u ttl=%d rtt=%.3f ms\n\r", 
