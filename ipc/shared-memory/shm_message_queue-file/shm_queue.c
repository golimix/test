...
inline void shmq_siguser1(int signo)
{
   (void)signo;
   snprintf(errmsg, sizeof(errmsg), "accept a sig\n");
}
 ...
 */
static int __attribute__((unused)) shmkey_exist(uint64_t shm_key);
 ...
 *  Returns a shm queue pointer or NULL if failed    
 */
sq_t shmqueue_create(const char *file, int ele_size, int ele_count)
{
	struct __shmqueue_head_t *queue;

    uint64_t shm_key = ftok(file, SQ_FTOK_PROJ_ID);

	if(ele_size<=0 || ele_count<=0 || shm_key<=0 || (!file)) // invalid parameter
	{
    ...
    snprintf(errmsg, sizeof(errmsg), "Success");
...
sq_t shmqueue_open(const char *file)
{
    uint64_t shm_key = ftok(file, SQ_FTOK_PROJ_ID);
...
int shmqueue_put(sq_t queue, void *data, int datalen)
{
    if(!queue)
    {
        snprintf(errmsg, sizeof(errmsg), "Null value.\n");
	    return -1;
    }
