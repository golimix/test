...
#define SHNQ_SIGUSR1    shmq_siguser1
/**
 *  Create a shm queue
 *  Parameters:
 *     shm_key      - shm key
 *     block_size   - preallocated size for each element 
 *     nblock       - preallocated number of elements   
 *  Returns a shm queue pointer or NULL if failed    
 */
sq_t shmqueue_create(const char *file, int block_size, int nblock);
