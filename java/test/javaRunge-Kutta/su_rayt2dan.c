...
int main(int argc, char **argv)
{
	...
	FILE *fpv,*fpd,*fpe,*fpt,*fpr,*fpp,*fpl;
	Ray *ray;

	/* input file */
	 char FNv[250]={"vel.dat"};     /*  velocity(nx*nz)  */
	 char FNd[250]={"delta.dat"};    /*  delta(nx*nz)  */
	 char FNe[250]={"epsilon.dat"};  /*  epsilon(nx*nz)  */
	/* output file */
        char FNt[250]={"time.dat"};          /*  Traveltime(nx*nz)  */ 
        char FNp[250]={"raypath.txt"};       /*  Raypath(x,z,t)  */
        char FNr[250]={"record.txt"};        /*  surface record(x,t)  */
        char FNl[250]={"length_a_l.txt"};    /*  Ray length(a,l)  */

	/* get parameters for input alloc */
	nt = 1501;
	nx = 301;	fx = 0.0;  dx = 5.0;
	nz = 301;	fz = 0.0;  dz = 5.0;
