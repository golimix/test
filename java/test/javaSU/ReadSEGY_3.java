...
class ReadSEGY{
        ...
        public static void main(String[]args)                 
                ...
                ArrayList<cjbsegy> hdrlist = ReadSEGY.readSegy("s.su");
                //for(cjbsegy hdr:hdrlist)
                //        System.out.println(hdr.cdp);

                //Field h = cjbsegy.class.getDeclaredField("sx");
                //h.setAccessible(true);

                //for(int itrace = 0;itrace<hdrlist.size();itrace++)
                //        System.out.println(" = "+h.get(hdrlist.get(itrace)));

                new mySEGYJFrame(hdrlist);
...
class mySEGYJFrame extends JFrame{
        ...
        mySEGYJFrame(final ArrayList<cjbsegy> hdrlist){
                ...
                Object[] columnNames = new Object[]{"Trace # "};
                DefaultTableModel model = new DefaultTableModel(columnNames, hdrlist.size());
                final JTable hdrtable = new JTable(model){
                        @Override
                ...
                JScrollPane hdrscrollpane = new JScrollPane(hdrtable);
